...
export interface Props {
  fallbackHttpRouterGroupsNames: string[];
...
export const FallbacksContainer = ({ fallbackHttpRouterGroupsNames, ...props }: Props) => {
  return (
    <FallbacksContainerView className={props.className}>
      {fallbackHttpRouterGroupsNames.map((fallbackGroupName, i) => {
        return (
          <FallbackContainer key={i}>
            ...
            {i < fallbackHttpRouterGroupsNames.length - 1 && <FontAwesomeIcon icon={Icons.CARET_RIGHT} />}
          </FallbackContainer>
        );
      })}
