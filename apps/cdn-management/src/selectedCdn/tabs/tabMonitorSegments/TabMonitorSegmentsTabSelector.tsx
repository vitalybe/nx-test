...
export const TabMonitorSegmentsTabSelector = (props: Props) => {
  const cdn = useSelectedCdn();
  const prepareQueryResult = MonitorSegmentProvider.instance.prepareQuery(cdn.id);
  const queryResult = prepareQueryResult.useQuery();

  const itemsCount = queryResult.data?.length;
  return (
    <TabSelector
      title={"Monitoring Segments"}
