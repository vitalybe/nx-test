...
import { MutableRefObject, ReactChild, ReactElement, ReactNode, useMemo, useRef, useState } from "react";
...
export const QwiltGrid = genericMemo(
  <T extends unknown>(props: Props<T>) => {
    ...
    // NOTE: AgGrid doesn't update the columns. So if we pass to it a react element, with a callback that callback will stay the same
    // during the lifetime of the grid. To handle that, we keep a ref to the given props. It allows us to "cheat" ag-grid and when it tries
    // to access the react renderer, we find the latest version, from the propsRef and return it instead. This assures that the data that was
    // captured by the closure of the callback is always current.
      ...
      <QwiltGridView
        ...
        <AgGridReact
          ...
          onGridSizeChanged={(event) => {
            setTimeout(() => {
              autoResizeColumns(event, props.suppressSizeColumnsToFit);
              // NOTE: This is needed to remove a useless horizontal scrollbar that sometimes lingers after auto-resize
              gridApiRef.current!.redrawRows();
              setIsRendering(false);
            }, 100);
          }}
          columnDefs={getColumnDefs(props.columns, props.actions || [], propsRef)}
          rowData={rows}
          ...
          autoGroupColumnDef={
            props.treeData?.treeColumn ? getColumnDef(props.treeData.treeColumn, propsRef, [], true) : undefined
          }
          onGridReady={(event) => {
...
function autoResizeColumns(data: { api: GridApi; columnApi: ColumnApi }, suppressSizeColumnsToFit?: boolean) {
...
function getCellRendererContainer<T>(
  ...
  const isTreeColumn = propsRef.current.treeData?.treeColumn === qwiltColumnDef;
  const disableOverflowTooltip = propsRef.current?.disableOverflowTooltip;
...
function findColumnInTree<T>(columns: QwiltGridBaseColumnDef<T>[], indexTree: number[]) {
  let parent: QwiltGridColumnDef<T> | undefined;
  if (indexTree.length > 1) {
    const leafIndex = indexTree.length - 1;
    parent = columns[indexTree[0]];
    for (let depth = 1; depth < leafIndex; depth++) {
      if (parent.children) {
        parent = parent.children[indexTree[depth]];
      }
    }
    return parent && parent.children && parent.children[indexTree[leafIndex]];
  } else if (indexTree.length === 1) {
    return columns[indexTree[0]];
  }
}
...
function getCellRenderer<T>(
  ...
  if (columnRenderer instanceof GridReactRenderer) {
    return (props: ReactRendererProps<T>) => {
      // NOTE: See explanation to see in declaration of propsRef
      let refColumn: QwiltGridColumnDef<T> | QwiltGridTreeColumnDef<T> | undefined;
      if (isTree) {
        refColumn = propsRef.current.treeData?.treeColumn;
      } else {
        refColumn = findColumnInTree(propsRef.current.columns, indexTree);
      }
      const renderer = refColumn?.renderer ?? qwiltColumnDef.renderer;

      if (renderer instanceof GridReactRenderer) {
        return renderer.reactRender({
          entity: props.entity,
          value: props.value,
    ...
    const renderer = columnRenderer.reactRender;
    return (props: ReactRendererProps<T>) =>
  ...
  return {
    ...
    cellRendererFramework:
      qwiltColumnDef.renderer instanceof GridReactRendererStringOnly
        ? undefined
        : getCellRendererContainer(qwiltColumnDef, propsRef),
    ...
    children:
      ...
      qwiltColumnDef.children.map((childColumn, i) => {
        return getColumnDef(childColumn, propsRef, [...indexTree, i]);
      }),
    ...
    cellRendererParams: {
      ...
      innerCellRenderer: getCellRenderer(qwiltColumnDef, propsRef, indexTree, isTree),
      suppressCount: true,
    },
...
function getColumnDefs<T>(
  ...
  const columns = propColumnDefs.map((column, i) => getColumnDef(column, propsRef, [i]));
  if (actions.length > 0)
    columns.push({
      headerName: "Actions",
      colId: "actions",
      ...
      cellRendererFramework: (props: ICellRendererParams) => {
        return (
          <ReactCellWrapper cellRendererParams={props}>
            <ActionContainer>
              {propsRef.current.actions?.map((action) => {
