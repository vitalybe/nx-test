...
export class XAxisBehavior implements ChartBehavior {
  ...
  private getTickPositions(points: { x: number; y: number | null }[]) {
    ...
    if (points.length > 0) {
      ...
      while (currentDate <= endDate) {
        ...
        // if we want to round but the current date is not rounded we adjust the delta so the next tick will appear at a rounded location
        if (this.roundTickEvery && currentDate.startOf(this.roundTickEvery).toMillis() < currentDate.toMillis()) {
          const nextRounded = currentDate.plus({ [this.roundTickEvery]: 1 }).startOf(this.roundTickEvery);
          const nextTickDelta = Duration.fromMillis(nextRounded.toMillis() - currentDate.toMillis());
          currentDate = currentDate.plus(nextTickDelta);
        } else {
  ...
  modifyConfig(chartOptions: Options, chartSeriesData: ChartSeriesData[]): void {
    ...
    if (points.length > 0) {
      ...
      const xDeltaDuration = this.roundTickEvery
        ? Duration.fromObject({ [this.roundTickEvery]: 1 })
        : Duration.fromMillis(xDelta);
    ...
    chartOptions.xAxis = {
      ...
      labels: {
        ...
        formatter(this: AxisLabelFormatterOptions): string {
          if (that.minLabelsGap) {
            const currentPosPixel = this.axis.toPixels(this.pos);
            const nextPosIndex = this.axis.tickPositions.indexOf(this.pos) + 1;
            const nextPosPixel = this.axis.toPixels(this.axis.tickPositions[nextPosIndex]);
            const isTooClose = nextPosPixel - currentPosPixel < that.minLabelsGap;
            if (isTooClose) {
              return "";
            }
          }
