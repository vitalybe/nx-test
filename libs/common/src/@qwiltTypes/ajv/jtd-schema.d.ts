...
export declare type SomeJTDSchemaType = (
  | // ref
  {
      ref: string;
    }
...
declare type IsUnion_<T, U extends T = T> = false extends (T extends unknown ? ([U] extends [T] ? false : true) : never)
  ? false
  : true;
...
declare type IsElements<T> = false extends IsUnion<T>
...
declare type IsValues<T> = false extends IsUnion<Exclude<T, null>>
...
declare type IsRecord<T, Union extends boolean> = Union extends IsUnion<Exclude<T, null>>
  ? null extends EnumString<keyof Exclude<T, null>>
    ? false
    : true
  : false;
/** actual schema */
export declare type JTDSchemaType<T, D extends Record<string, unknown> = Record<string, never>> = (
...
declare type JTDDataDef<S, D extends Record<string, unknown>> =
  | (S extends {
      ref: string;
    }
      ? D extends {
