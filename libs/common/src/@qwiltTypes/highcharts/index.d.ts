...
declare namespace Highcharts {
  ...
  interface AxisLabels {
    ...
    /**
     * Polar _parts only. The label's pixel distance from the perimeter of the plot area.
     * @default 15
     */
    distance?: number;
    /**
  ...
  interface PlotBands {
     ...
     * tickLength, thus making the plot band act as a background for the tick bubbleMarkers.
     * Only defined for yAxis.plotBands
     * @default 10
     * @since 2.3
     */
  ...
  interface AnnotationsShape extends AnnotationsShapeOptions {
    /**
     * Id of the marker which will be drawn at the final vertex of the path. Custom bubbleMarkers can be defined in defs property.
     * @default undefined
     * @since 6.0.0
     */
    markerEnd?: string;
  ...
  interface AccessibilityOptions {
    /**
     * Whether or not to add series descriptions to _parts with a single series. Defaults to false.
     * @since 5.0.0
     */
  ...
  interface AxisOptions {
    ...
    /**
     * The amount of ticks to draw on the axis. This opens up for aligning the ticks of multiple _parts or panes within
  ...
  interface ChartOptions3d {
    ...
    enabled?: boolean;
    /**
     * Provides the option to draw a frame around the _parts by defining a bottom, front and back panel.
     */
    frame?: {
      /**
       * Defines the back panel of the frame around 3D _parts.
       * @since 4.0
       */
     ...
     * perspective effect in column and scatter _parts. It is not used for 3D pie _parts.
     * @default 100
     * @since 4.0
     */
    viewDistance?: number;
  ...
  interface ChartOptions {
    ...
    borderWidth?: number;
    /**
     * A CSS class name to apply to the _parts container div, allowing unique CSS styling for each chart.
     */
    className?: string;
    /**
    ...
    marginTop?: number;
    /**
     * Options to render _parts in 3 dimensions. This feature requires highcharts-3d.js, found in the download package
     ...
     * setting. However, pinching can be enabled separately in some cases, for example in stock _parts where a mouse
     ...
     * CSS background image to the container element. Note that for the image to be applied to exported _parts, its URL
    ...
    /**
     * When true, cartesian _parts like line, spline, area and column are transformed into the polar coordinate system.
    ...
    /**
     * Whether to show the axes initially. This only applies to empty _parts where series are added dynamically, as axes
     ...
     * Highcharts will by default attempt to guess the chart type, but for more complex _parts it is recommended to specify this property for clarity.
     * @since 5.0.0
     * @default undefined
     */
  ...
  interface DataOptions {
    ...
    /**
     * Item or cell delimiter for parsing CSV. Defaults to the tabRouter character \t if a tabRouter character is found in the CSV
  ...
  interface ExportingOptions {
    ...
    url?: string;
    /**
     * The pixel width of _parts exported to PNG or JPG. As of Highcharts 3.0, the default pixel width is a function of
  ...
  interface GlobalObject {
     ...
     * _parts in specific time zones using their local DST crossover dates, with the help of external libraries.
     * @since 4.1.0
     */
  ...
  interface LegendNavigationOptions {
    ...
    activeColor?: string;
    /**
     * How to animate the tabs when navigating up or down. A value of true applies the default navigation given in the
  ...
  interface LegendOptions {
     ...
     * point object in case of pie _parts. By default the series or point name is printed.
     */
  ...
  /**
   * Applies only to polar _parts and angular gauges. This configuration object holds general options for the combined X
  ...
  interface MarkerState {
    /**
     * Enable or disable the point marker. If null, the bubbleMarkers are hidden when the data is dense, and shown for more
  ...
  interface Marker extends MarkerState {
    /**
     * Image bubbleMarkers only. Set the image width explicitly. When using this option, a width must also be set.
     * @default null
     * @since 4.0.4
     */
    height?: number | null;
     ...
     * applied to exported _parts, its URL needs to be accessible by the export server.
     *
    ...
    /**
     * Image bubbleMarkers only. Set the image width explicitly. When using this option, a height must also be set.
     * @default null.
     * @since 4.0.4
     */
    width?: number | null;
  ...
  interface Halo {
    ...
    opacity?: number;
    /**
     * The pixel size of the halo. For point bubbleMarkers this is the radius of the halo. For pie slices it is the width of
  ...
  interface LineStates {
     ...
     * in pie _parts. By default the halo is filled by the current point or series color with an opacity of 0.25. The
  ...
  interface SeriesChart {
    /**
     * Allow this series' points to be selected by clicking on the bubbleMarkers, bars or pie slices.
     * @default false
     * @since 1.2.0
     */
    ...
    className?: string;
    /**
     * The main color or the series. In line type series it applies to the line and the point bubbleMarkers unless otherwise
    ...
    color?: string | Gradient;
    /**
     * Polar _parts only. Whether to connect the ends of a line series plot across the extremes.
     * @default true
     * @since 2.3.0
     */
    connectEnds?: boolean;
     ...
     * outside the visible plot area at the current zoom. The advantage of drawing all points (including bubbleMarkers and
    ...
    dashStyle?: string;
    /**
     * Gantt _parts use one or more data labels for each series, for showing multiple date periods.
     */
     ...
     * it needs to be set.
     *
     * Defaults to null in cartesian _parts, 'between' in polar _parts.
     * @since 2.3.0
     */
     ...
     * mouse leaves the area around the series' graph or bubbleMarkers. This also implies the tooltip. When stickyTracking is
  ...
  interface BarChart extends SeriesChart {
     ...
     * close to zero) point, set the minimal point length to a pixel value like 3. In stacked column _parts,
  ...
  /**
   * Funnel _parts are a type of chart often used to visualize stages in a sales project, where the top are the initial
  ...
  interface IndividualSeriesOptions {
     ...
     */
    data?: Array<
      number | [number, number] | [string, number] | [string, number, number] | [number, number, number] | DataPoint
    >;
    /**
  ...
  interface TimeOptions {
     ...
     * January 1 1970, and returns the timezone offset in minutes. This provides a hook for drawing time based _parts in
  ...
  interface TooltipOptions extends SeriesTooltipOptions {
     ...
     * the tooltip being aligned inside the chart area. For small _parts, this may result in
     ...
     * recommended for single series _parts and for tablet/mobile optimized _parts.
     * @default false
     * @since 2.1
     */
    shared?: boolean;
     ...
     * This is recommended over shared tooltips for _parts with multiple line series,
  ...
  interface Options {
    ...
    /**
     * Applies only to polar _parts and angular gauges. This configuration object holds general options for the combined
  ...
  interface ChartObject {
    ...
    plotWidth?: number;
    tooltip: {
      // NOTE: the function expects an array if the tooltip is in shared mode (single tooltip for all series)
      refresh: (point: PointObject | PointObject[]) => void;
    };
   ...
   * the Renderer object, these elements allows freeform annotation in the _parts or even in your HTML tabs without
  ...
  interface RendererObject {
    ...
    path(attrib: SVGAttributes): ElementObject;
    /**
     * Add a rectangle.
     ...
     * @param useHtml Use HTML to render the text, default "false"
     * @since 2.0
     */
    text(str: string, x: number, y: number, useHtml?: boolean): ElementObject;
  }
  ...
  interface Static {
    ...
    /**
     * Sets the options globally for all _parts created after this has been called. Takes an options JavaScript object
     ...
     */
    addEvent(element: HTMLElement | ElementObject | object, type: string, cb: (evt: Event) => void): () => void;
  ...
  interface PointObject {
    ...
    /**
     * The total of a stack for stacked series, or pie in pie _parts.
     */
    total: number;
    /**
  ...
  interface SeriesObject {
     ...
     * Use this option for live _parts monitoring a value over time.
  ...
  /**
   * The Tooltip prototype is used to to wrap and extend tooltip _chartBehaviors.
   *
   * @see {@link https://www.highcharts.com/docs/extending-highcharts/extending-highcharts}
   */
